...
const Graph = ({range}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/dashboard/graph?${range}`, { withCredentials: true })
   ...
   return (
    <div style={{ width: '100%', height: '100%', minHeight: '350px' }}>
      <LineChart
        ...
        yAxis={[{
            valueFormatter: (value) => `₹${Intl.NumberFormat('en-IN', { notation: 'compact', maximumFractionDigits: 1 }).format(value)}`,
        }]}
        series={[
          {
